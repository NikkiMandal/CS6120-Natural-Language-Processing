...
checkpoint_for_adapter = "xlnet/xlnet-base-cased"
...
class XLNetClassificationHead(nn.Module):
...
class XLNetForSequenceClassification(XLNetPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.attention = MultiheadAttention(
            self.xlnet.config.hidden_size, num_heads=8, batch_first=True
        )
...
def save_results(config, label_encoder, predictions, test_df):
    ...
    outputs_dict[
        "note"
    ] = "xlnet_model_with_attention_check_cbr_different_features_for_retrieval"
...
def do_train_process(config=None):
    with wandb.init(config=config):
        ...
        if config.eval_only:
            tokenizer = XLNetTokenizer.from_pretrained(config.model_dir)
            model = XLNetForSequenceClassification.from_pretrained(config.model_dir)
        else:
            tokenizer = XLNetTokenizer.from_pretrained(checkpoint_for_adapter)
        ...
        training_args = TrainingArguments(
            do_eval=True,
            do_train=True,
            output_dir=f"models/cbr_xlnetlogical_fallacy_classification_{config.data_dir.replace('/', '_')}",
            save_total_limit=2,
        ...
        if not config.eval_only:
            trainer.train()
            trainer.save_model(
                f"models/cbr_xlnet_logical_fallacy_classification_{config.data_dir.replace('/', '_')}_feature_{config.feature}"
            )
